...
/**
 * Declares the extension on every outgoing call via the `A2A-Extensions` header
 */
...
async function createAndProvisionCredoAgent(): Promise<CredoAgentWithOpenId4Vc> {
...
async function handleAuthRequest(credoAgent: CredoAgentWithOpenId4Vc, event: TaskStatusUpdateEvent): Promise<boolean> {
  ...
  for await (const response of stream) {
    ...
    if (payload.$case === 'statusUpdate') {
      ...
      if (state === TaskState.TASK_STATE_AUTH_REQUIRED) {
        const presented = await handleAuthRequest(credoAgent, statusEvent)
        if (!presented) {
